...
module.exports = function (args, callback) {
  ...
  // 更新文章front-matter内容
  router.post('/post/edit/:postId', async (ctx, next) => {
    const params = ctx.request.body;
    const _id = ctx.params.postId;
    const post = hexo.model('Post').get(_id);
    const yfmData = yfm.loadFront(post.raw);
    const fileName = post.source;
    const content = post._content;
    const postPath= path.resolve(__dirname, '..', '..', 'doc-server', 'source', fileName);

    // 保留原始创建日期
    params.date = post.date.format('YYYY-MM-DD HH:mm:ss');
    delete yfmData.__content;

    ctx.body = new Promise((resolve, reject) => {
      const updateContent = `---\n${jsYaml.dump({...yfmData, ...params})}---\n${ content}`;

      fs.writeFile(postPath, updateContent, (err) => {
        if (err) {
          console.log(err)
          reject(err)
        }
        resolve({
          data: `${ _id } updated done ...`
        })
      })
    });
  ...
  // 更新文章内容
  router.post('/post/updateContent/:postId', async (ctx, next) => {
    const params = ctx.request.body;
    const _id = ctx.params.postId;
    const post = hexo.model('Post').get(_id);
    const yfmData = yfm.loadFront(post.raw);
    ...
    ctx.body = new Promise((resolve, reject) => {
      const updateContent = `---\n${jsYaml.dump(yfmData)}---${ content.replace(/^\n*/m, '\n') }`;
